  ...
  try {
    ...
    if (response.data[0]) {
      ...
      */
      ...
      Alert.alert('Meetup updated', 'Meetup successfully updated!');
      ...
      Alert.alert(
        'Update meetup error',
        'Unable to update the meetup! Verify your data!'
      );
    ...
    if (err.response) {
      Alert.alert('Update meetup error', `Error: ${err.response.data.error}`);
...
export function* addMeetup({ payload }) {
  try {
    ...
    Alert.alert('Meetup created', `Meetup '${title}' successfully created!`);
    yield put(addMeetupSuccess());
    // history.push('/dashboard');
  } catch (err) {
    if (err.response) {
      Alert.alert('Create meetup error', `Error: ${err.response.data.error}`);
      console.tron.error(err.response.data.error);
    } else {
      Alert.alert(
        'Create meetup error',
        'Unable to create the meetup! Verify meetup data!'
      );
      console.tron.error(err);
    }
...
export function* cancelMeetup({ payload }) {
  try {
    ...
    Alert.alert('Meetup canceled', `Meetup '${title}' successfully canceled!`);

    yield put(cancelMeetupSuccess());
    // history.push('/dashboard');
  } catch (err) {
    if (err.response) {
      Alert.alert('Cancel meetup error', `Error: ${err.response.data.error}`);
      console.tron.error(err.response.data.error);
    } else {
      Alert.alert('Cancel meetup error', 'Unable to cancel the meetup!');
      console.tron.error(err);
    }
